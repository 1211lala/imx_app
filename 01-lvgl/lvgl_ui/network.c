...
#define IMX_IP_ADDR "192.168.28.115"
#define IMX_IP_PORT 8000
...
void *network_task(void *arg)
{
    ...
    inet_pton(AF_INET, IMX_IP_ADDR, &client.sock.sin_addr);
    client.sock.sin_family = AF_INET;
    client.sock.sin_port = htons(IMX_IP_PORT);
    /* 绑定TCP客户端参数 */
    ...
    inet_pton(AF_INET, "192.168.28.103", &client.s_sock.sin_addr);
    client.s_sock.sin_family = AF_INET;
    client.s_sock.sin_port = htons(8080);
    /* 开始连接 */
    ...
    while (1)
    {
        send(client.sockfd, "this is send()\r\n", strlen("this is send()\r\n"), 0);
        sleep(1);
        write(client.sockfd, "this is write()\r\n", strlen("this is write()\r\n"));
        sleep(1);
    }
}
